...
$$ where $L$ is the numbers of generation links between two individuals ($i$ and $j$) through the shortest pathways over each common ancestor. E.g. full siblings are linked by two links through the mother (siblingA - mother - siblingB) and two links through the father (siblingA - father - siblingB), therefore the coefficient of relationship between them is $r = (^1/_2)^2 + (^1/_2)^2 = (^1/_4) + (^1/_4) = (^1/_2)$ (**Figure 1A**), while e.g. a person with their aunt are linked by three links through the shared grandmother/mother and three through the shared grandfather/father, so $r=(^1/_2)^3 + (^1/_2)^3 = (^1/_8) + (^1/_8) = (^1/_4)$ (**Figure 1B**).\

![**Figure 1.** Generation links count for the coefficient of relationship calculation. A) Number of links between two siblings comprises two links through the mother (red) and two links through the father (green). B) Number of links between a person and their aunt comprises three links through the shared grandmother/mother (red) and three through the shared grandfather/father (green).](CoefficientOfRelationship.png){fig-align="center" width="60%"}

Note that under such definition, the coefficient of relationship is a lower bound and an actual value that may be up to a few percent higher due to unaccounted for consanguinity within the pedigree.
...
$$
\phi_{ij} = (^1/_2)r_{ij}
$$ and hence provides a direct relationship between the degrees of relatedness from the pedigree and the expected kinship coefficient $\phi_{ij}$  (**Table 1.** ).


| Relationship | Degree | Coefficient of relationship ($r_{ij}$) | Kinship coefficient ($\phi_{ij}$) |
|-----------------:|:----------------:|:----------------:|:----------------:|
| **Identical Twins / Same Individual** | $0$ | $2^{0} = 1$ | $1/2$ |
| **Parent-Offspring** | $1$ | $2^{-1} = 1/2$ | $1/4$ |
| **Sibling-Sibling** | $1$ | $2 \cdot 2^{-2} = 1/2$ | $1/4$ |
| **Grandparent-Grandchild** | $2$ | $2^{-2} = 1/4$ | $1/8$ |
| **Aunt/Uncle - Niece/Nephew** | $2$ | $2 \cdot 2^{-3} = 1/4$ | $1/8$ |
| **Half-Siblings** | $2$ | $2^{-2} = 1/4$ | $1/8$ |

: **Table 1.** Values of the coefficient of relatedness and the kinship coefficient for different pedigree relationships up to the second-degree, assuming that $C_I$ = 0. From Rohrlach et al. [-@Rohrlach2023.04.17.537144].

## Pairwise Mismatch Rate (PMR)

Methods of relatedness estimation routinely used on modern data are not applicable to ancient genomes due to small numbers of individuals sampled and high rate of data missingness (i.e., low coverage), as well as due to lack of diploid phased genomic data available for majority of such samples. Thus, estimation of the coefficient of relatedness for ancient individuals using these methods is prone to biases and generally unreliable.

Pairwise mismatch rate (PMR) was introduced by [@Kennett2017-qw] as a means to estimate relatedness between ancient individuals (**Figure 2**). For each pair of individuals they computed the average mismatch rate across all autosomal SNPs covered by at least one sequence read for both of the two compared individuals (when \>1 sequence read was present for one individual at a given site, a random read was sampled for the analysis) and computed standard errors using a weighted block jackknife. Mismatch rates significantly lower (Z\>3) than the highest observed value, provided putative evidence of relatedness.
...
![**Figure 2.**Pairwise mismatch rate calculation (PMR). A) Pseudohaploid genomes are compared within each pair of individuals. B) Only sites called in both individuals are considered (filled circles) and classified as match (green) or mismatch (red). Accounting for linkage disequilibrium: C) Estimation of PMR using sliding window (as implemented in READv1) as well as genome-wide (optional in READv2). D) Estimation of PMR on thinned SNP data (as implemented in BREADR)](PMRcalculation.jpg){fig-align="center" width="500"}
...
The estimate of relatedness coefficient $r$ needs therefore to be corrected using the expected mismatch rate in non-related individuals. In [@Kennett2017-qw] they chose correction based on the approximate maximum mismatch rates observed: $$
b = max(PMR_{observed})/2 .
$$ The estimator they used is thus: $$
r = 1 - ((PMR_{ij} - b)/b) .
$$

PMR estimation in ancient-DNA-based inference of relatedness has first been implemented as separate software with READ [@Monroy_Kuhn2018-fb] and then by its successor - READv2 [@Alacamli2024-xq]. Other software used in relatedness estimation among ancient individuals, such as BREADR [@Rohrlach2025-va], also build on PMR.

### Accounting for linkage disequilibrium

Linkage disequilibrium (LD; non-independent co-inheritance) of the loci included in the PMR estimation will bias the results. The non-independence of loci will lead to underestimation of the variance (standard error) of the PMR and hence falsely inflate the confidence of relatedness estimates. Moreover, large shared blocks packed with unpruned SNPs could artificially pull down the overall PMR making individuals look more related than they are, while dense regions of high local diversity could inflate the PMR estimate making individuals look less related.\
To minimize this effect, variuos approaches can be employed. This is particularly crucial, when analyzing genome-wide (shotgun) data. In the 1240k SNP panel widely used in ancient genomics the analysed loci have already been selected taking LD into account. The approaches to minimize the LD bias that have been employed in PMR estimation comprise PMR estimation separately over consecutive genome fragments (sliding window) and obtaining median estimate among them (e.g., implemented in READv1, and as an option in READv2; **Figure 2C**), and decreasing the number of SNPs included in the analysis using a threshold of physical proximity of SNPs along the genome (thinning; e.g., implemented in BREADR; **Figure 2D**).
...
**R**elationship **E**stimation from **A**ncient **D**NA [@Monroy_Kuhn2018-fb] - formalized implementation of PMR for ancient genomic data.

- Pseudohaploid input data ([TPED/TFAM](https://www.cog-genomics.org/plink/1.9/formats#tped) format)
...
- Normalization of *P*0 using the value expected for a randomly chosen pair of unrelated individuals from the population (in order to make the classification independent of within population diversity, SNP ascertainment and marker density) (**Figure 3**).\
...
![**Figure 3**. Outline of the general READ workflow to estimate the degree of relationship between two individuals. From](READ1.JPG){fig-align="center" width="60%"}
...
- Genome-wide *P*0 calculation as default (**Figure 4**); with the uncertainty estimated using a block-jackknife approach with block sizes of 5 Mb (as commonly employed in human population genomic studies; [@Patterson2012-im]). Option to use window-based approach of chosen window size also available.\
\
According to the authors, READv2 alone can lead to very similar results as the combination of READv1 and lcMLkin. Output table includes information such as the number of overlapping SNPs, the number of effectively overlapping SNPs, and the kinship coefficient $μ$ (= 1 - normalized *P*0)


![**Figure 4.** READv2 flowchart. From Alaçamlı et al. [-@Alacamli2024-xq] ](Alacamli.F5.Flowchart.jpg){fig-align="center", width="80%"}
...
The two types of relations are well separated down to 0.5x coverage in the simulated dataset (or ∼8,000 "effectively overlapping SNPs"), but they overlap at 0.2x and below. Hence, to avoid wrongly classifying parent-offspring pairs as siblings, READv2 applies a default cutoff of 10,000 effectively overlapping SNPs, below which classification is not performed. (In Rivollat et al. [-@Rivollat2020-qq] a cutoff of 7,000 effective SNP number cutoff was used.)

### Warnings

At low coverage (0.05x and 0.1x) there are high false positive rates for second- and third-degree relatedness; many unrelated pairs are classified into these categories. At 0.01x, unrelated individuals are even classified as first-degree or identical twins, resulting in a reduced false positive rate for second- and third-degree but an increased false positive rate for first-degree. To avoid false classifications in empirical data, READv2 applies a conservative threshold of 5,000 "effectively overlapping SNPs", below which no attempt to classify third-degree relatives is taken.


### Comparison with other methods

READv2 is very similar in its approach to BREADR [@Rohrlach2025-va] and TKGWV2 [@Fernandes2021-ze], with each tool having its own unique features. READv2 has the functionality to separate the different first-degree relationships, BREADR has a better quantification of uncertainty, and TKGWV2 works well with lower amounts of input data (**Table 2**).


| Method | Reference | Min. Coverage | Size of SNP panel used | Degree up to | Within Degree Differentiation | Type of Data Needed |
|:---------:|:---------:|:---------:|:---------:|:---------:|:---------:|:---------:|
| **lcMLkin** | [@Lipatov2015-uo] | 2x | 100,000 | Third | First degree (parent-offspring, siblings)‡ | Biallelic genotypes, genotype likelihoods and population allele frequencies\* |
| **ngsRelate** | [@Korneliussen2015-ub] | 1x | 100,000 | Third | First degree, some second degree (half-siblings) and some third degree (first cousins)‡ | Genotype likelihoods and population allele frequencies\*\* |
| **TKGWV2** | [@Fernandes2021-ze] | 0.026x | \~22,000,000 | Second | \- | Pseudohaploid genotypes and population allele frequencies |
| **KIN** | [@Popli2023-ny] | 0.05x | Not specified | Third | First degree | Aligned reads\*\*\* (.bam files) |
| **BREADR** | [@Rohrlach2025-va] | 0.04x | \~29,000,000 | Second | \- | Pseudohaploid genotypes |
| **ancIBD** | [@Ringbauer2024-kz] | 0.25x (WGS) or 1x (1240k SNP capture) | \~1,200,000 | Sixth | First and second degree | Imputed genotypes\*\*\*\* |
| **correctKin** | [@Nyerki2023-af] | \~85,000 overlapping markers† | \~1,200,000 | Fourth | \- | Pseudohaploid genotypes |
| **READv1** | [@Monroy_Kuhn2018-fb] | 0.1x | \~1,200,000 | Second | \- | Pseudohaploid genotypes |
| **READv2** | [@Alacamli2024-xq] | 0.05x or \~500 SNPs (first degree)<br>0.1x or \~2,000 SNPs (second degree)<br>0.3x or \~15,000 SNPs (third degree) | 200,000 | Third | First degree | Pseudohaploid genotypes |

> \*Estimated from the input data\
> \*\*Optional, can be estimated from the data
> \*\*\*The required input files for KIN (the number of overlapping sites, the number of pairwise differences, and the probability of runs of homozygosity (ROH) in the windows where individuals have the same long allele sequence) are created by KINgaroo, which is provided together with the KIN package, by using these .bam files.\
> \*\*\*\*Imputed genotypes are created with GLIMPSE [@Rubinacci2021-tj] by using aligned sequence data (.bam files) as a part of the pipeline.\
> † The authors of correctKin report their results with the term "overlapping markers", which stands for the percentage of markers shared by both genotypes.\
> ‡ This classification is not automatically performed by the tool but can be achieved by comparing different statistics against each other.

: **Table 2.** Comparison of methods available for relatedness reconstruction in ancient genomic data. From Alaçamlı et al. [-@Alacamli2024-xq].
...
The input for [READ](https://bitbucket.org/tguenther/read/src/master/) is a pair of files in Plink's [TPED/TFAM](https://www.cog-genomics.org/plink/1.9/formats#tped) (PLINK transposed text genotype table and sample information file).
...
" > myGenotypeFile.GenotToBed.convertf.param
...
- [trident genoconvert](https://www.poseidon-adna.org/#/trident) and [plink](https://www.cog-genomics.org/plink/) transpose, if you are working with [Poseidon](https://www.poseidon-adna.org/#/) packages:

``` bash
## Convert eigenstrat-formatted package(s) to plink-formatted
trident genoconvert -d ... -d ... --outFormat PLINK

## Transpose plink to tped
plink --bfile myGenotypeFile --recode transpose --out myGenotypeFile 
## The output is myGenotypeFile.tped, myGenotypeFile.tfam, myGenotypeFile.nosex, myGenotypeFile.log
```

With the generated PLINK files you can now run READ:
...
![](READ_results_plot.jpg){fig-align="center"}
...
The input for READv2 is a trio of files in Plink's [BED/BIM/FAM](https://www.cog-genomics.org/plink/1.9/formats#bim) format.

If starting with files in EIGENSTRAT format, they can be converted to PLINK format using for example:

- convertf ([Eigensoft](https://github.com/DReichLab/EIG)):

``` bash
## Create convertf parameter file for converting .geno to .bed files:
echo "
genotypename:    myGenotypeFile.geno
snpname:         myGenotypeFile.snp
indivname:       myGenotypeFile.ind
...
snpoutname:      myGenotypeFile.bim
indivoutname:    myGenotypeFile.fam
" > myGenotypeFile.GenoToBed.convertf.param
...
- `-i`, `--input_file \<value\>` -- Input file prefix (required). The current READ version only supports Plink bed/bim/fam files.

- `-n`, `--norm_method \<value\>` -- Normalization method (either 'mean', 'median', 'max' or 'value').
...
- `--norm_value \<value\>` -- Provide a user-defined normalization value

- `--window_size \<value\>` -- Change window size for block jackknife or for window-based P0 estimates (as in READv1), default: 5000000
...
![](READv2_result_plot.jpg){fig-align="center"}
